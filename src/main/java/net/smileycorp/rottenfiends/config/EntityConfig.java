...
public class EntityConfig {
    ...
    public static void syncConfig(FMLPreInitializationEvent event) {
        ...
        try{
            config.load();
            //excavator
            excavatorZombie = new EntityAttributesEntry(config, "Excavator Zombie", 0.153333336, 35, 6, 12, 1, 0, 0);
            ...
            excavatorReach = config.get("Excavator Zombie", "reach", 4, "How close do excavator zombies need to be to block to mine them.").getDouble();
        } catch(Exception e) {
        } finally {
